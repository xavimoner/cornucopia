# scripts/extract_single_comparison_csv.py
...
def truncate_to_tokens(text, max_tokens):
    words = text.split()
    approx_limit = max_tokens * 0.75
    return ' '.join(words[:int(approx_limit)])
...
def save_to_csv(results, convocatoria_id):
    ...
    filename = os.path.join("results", f"comparativa_models_convocatoria_{convocatoria_id}.csv")
